 ...
 */
	...
	#pragma omp parallel private(tmp_seq_matches)
	{
		tmp_seq_matches = (int*) calloc(sizeof(int), seq_length);		//private

		#pragma omp for \
			reduction(+: pat_matches)
		for( pat=0; pat < pat_number; pat++ ) {
			...
			if ( pat_found[pat] != (unsigned long)NOT_FOUND ) {
				...
				for( ind=0; ind<pat_length[pat]; ind++) {
					tmp_seq_matches[ pat_found[pat] + ind ] ++;
				}
			}
		}
		...
		for(ind=0; ind<seq_length; ind++) {
			#pragma omp atomic
			seq_matches[ind] += tmp_seq_matches[ind];
		}
		free(tmp_seq_matches);
	}
